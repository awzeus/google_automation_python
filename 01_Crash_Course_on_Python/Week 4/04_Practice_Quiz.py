...
for filename in filenames:
    if ".hpp" in filename:
        newfilenames.append(filename[:filename.index(".hpp")] + ".h")
    else:
...
def octal_to_string(octal):
    ...
    for individual_octal in [int(n) for n in str(octal)]:
        # Check for each of the permissions values
        for value, letter in value_letters:
            if individual_octal >= value:
                result += letter
                individual_octal -= value
            else:
                result += "-"
    return result
...
def group_list(group, users):
    ...
    complete_string = "{}: {}".format(group,members)
